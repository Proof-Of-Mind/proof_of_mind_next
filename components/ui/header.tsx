...
export default function Header() {
  ...
  const { address, isConnected, check, connect, disconnect } = useContext(ConnectContext);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex md:grow">
            ...
            <ul className="flex grow justify-end flex-wrap items-center">
              <li className='ml-3'>
                <button className="btn-sm text-white bg-purple-600 hover:bg-purple-700 mx-3 min-w-[132px]" onClick={handleConnectWallet}>
                  {isConnected ? address.substr(0, 6) + '...' + address.substr(-4) : 'Connect Wallet'}
                </button>
              </li>
            </ul>
          </nav>
